...
지금 상태:  Docker 컨테이너 2개 실행 중  +  kind 클러스터(Cilium v1.20.0) 실행 중
            예제 01~14 전부 "빈칸 채우고 채점받는" 챌린지 형식 + 랩별 학습 문서
            바로 시작:  cd ebpf-lab && make list
```

**전부 채우고 점수를 따는 구조다.** 읽어서는 안 는다.

```
make hunt        00. 훅 사냥 — 코드 없이 "어디에 붙일지" 만 먼저 훈련한다
make hunt-check  존재 검증 + 정답 검증 (10문제)
   ↓
make NN          예제 실행 — 빈칸 상태라 결과가 안 나온다
   ↓             examples/NN-*/*.bpf.c 의 🎯 TODO 를 Lv0 → Lv1 → Lv2 순서로 채운다
make NN-check    단계별 자동 채점 (앞 단계를 통과해야 뒤 단계를 본다)
   ↓
open examples/NN-*/STUDY.html    이론 정리 + 동작 해부 + OX 퀴즈로 마무리
   ↓
make NN+1
```

**훅 선택이 eBPF 개발의 8할이다.** 그래서 `00. 훅 사냥`을 맨 앞에 뒀다.
01·02 는 훅을 미리 주고 개념에 집중하지만, **03 번부터는 `SEC()` 이 비어 있다** —
어디에 붙일지 직접 찾아야 Lv0 를 통과한다.

- **01~07 기본기** — kprobe · tracepoint · fentry/fexit · uprobe · XDP · TCX · 실무형 에이전트
- **08~14 보안 게임** — 변조 · 탐지 · LSM 차단 · DDoS 방어 · 안티치트 · 백신 · IDS/IPS
  전부 loopback 안에서 벌어지는 자가 검증이라 외부 대상이 없다.
...
**기본기 트랙 (00~07)**
0. [00. 훅 사냥 — 코드 전에 훅부터](#0-훅-사냥--코드-전에-훅부터)
...
```bash
make hunt          # 목표 10개 (examples/00-hunt/TARGETS.md)
# examples/00-hunt/answers.txt 를 채운다
make hunt-check    # 채점
```

**코드를 한 줄도 안 쓴다.** "이 이벤트를 관측하려면 어디에 붙어야 하나" 만 찾는다.

잘못된 훅을 고르면 코드가 아무리 좋아도 소용없는데, 그 사실은 **전부 짜고 나서야**
드러난다. 그래서 순서를 뒤집었다 — **찾고, 존재를 확인하고, 그 다음에 짠다.**

채점은 두 단계다:

1. **존재 검증** — 적어낸 훅이 **이 커널에** 실제로 있는가.
   없으면 정답 여부를 보기 전에 걸린다.
2. **정답 검증** — 목표에 맞는 훅인가. (정답이 여러 개인 문제도 있다)

> 1번이 진짜 목적이다. 실무에서 가장 흔한 실패는 "틀린 훅을 골랐다"가 아니라
> **"그 커널에 없는 훅을 가정하고 짰다"** 이다. 블로그에서 본 함수 이름을 그대로
> 썼는데 우리 커널엔 없거나 이름이 다른 경우.

목표에는 tracepoint / kprobe / fentry / uprobe / LSM / TC 가 골고루 섞여 있고,
**"차단하려면 어떤 훅이어야 하나"**, **"egress 는 왜 XDP 로 안 되나"** 같은
선택 기준을 직접 답하게 만든다.

정리와 퀴즈: `examples/00-hunt/STUDY.html`
...
**01~07 도 08~14 와 똑같이 빈칸을 채우고 채점받는다.** 읽기만 하면 안 남는다.

### 랩 하나의 구조

```
examples/01-kprobe-unlink/
├── unlink.bpf.c              ← 네가 채우는 게임판 (🎯 Lv1~Lv4 TODO)
├── main.go                   ← 완성본 (로더는 보일러플레이트다. 건드리지 않는다)
├── check.sh                  ← 단계별 자동 채점
├── solution/unlink.bpf.c     ← 정답 (막히면)
└── STUDY.html                ← 이론 + 동작 해부 + OX 퀴즈  ← 채점 통과 후 열어본다
```

**왜 `main.go` 는 완성본으로 주나:** 로더 코드는 14개 예제에서 80% 가 똑같다.
실력이 갈리는 건 **어느 훅에 붙일지, 커널에서 뭘 계산할지, 데이터를 어떻게 내보낼지**이고
그건 전부 `.bpf.c` 쪽이다. 다만 **디버깅할 때는 `main.go` 를 읽어야 한다** —
verifier 에러도 attach 실패도 전부 로더 쪽에서 튀어나온다.

### 03 번부터는 훅도 직접 찾는다

01·02 는 `SEC()` 을 주고 개념(링버퍼·맵·집계)에 집중한다.
**03 번부터는 `SEC("TODO")` 로 비어 있다.** 안 채우면 로드 자체가 실패한다:

```
BPF 오브젝트 로드 실패: ... program type is unspecified
```

섹션 이름이 곧 프로그램 타입이기 때문이다. 각 `.bpf.c` 의 `🎯 Lv0` 주석에
**정답 대신 탐색 명령**이 들어 있다. 채점기는 "부착 성공" 을 Lv0-b 로 따로 채점한다.

### 진행 방법

```bash
make 01          # 빈칸 상태로 실행 (아무 이벤트도 안 나온다)
make 01-check    # 15/100  — Lv0 빌드만 통과
# unlink.bpf.c 의 🎯 Lv1 TODO 를 채운다
make 01-check    # 40/100  — Lv1 통과, Lv2 부터 채점 시작
# ... Lv3 까지 채우면 90/100, 통과 기준은 70점
open examples/01-kprobe-unlink/STUDY.html   # 이론 정리 + OX 퀴즈
```

**앞 단계를 통과해야 뒤 단계를 채점한다.** 건너뛰면
"내 로직이 틀린 건지 verifier가 이 패턴을 싫어하는 건지" 구분이 안 돼서 막히기 때문이다.

### 각 랩의 학습 문서 (STUDY.html)

채점을 통과한 뒤에 열어본다. 브라우저에서 바로 열리는 단일 HTML 파일이다.

```bash
make study                                   # 목록 보기
open examples/01-kprobe-unlink/STUDY.html
```

담긴 것: **그 훅의 원리**(왜 이게 필요한가, 무엇과 거래하는가) · **이 랩의 동작 해부** ·
**단계별 공략과 채점 기준** · **함정** · **OX 퀴즈 + 빈칸 채우기**(클릭하면 즉시 채점 + 해설).
02번 문서에는 **eBPF 자체를 처음부터 정리한 00절**이 따로 있다 —
verifier·맵·헬퍼·훅 지도·CO-RE·eBPF가 못 하는 것까지. 처음이면 여기부터 읽는 게 빠르다.
...
01~07 이 훅과 맵의 **기본기**를 하나씩 익히는 트랙이라면, 08~14 는 그걸로
**변조하고, 차단하고, 탐지**하는 트랙이다. 관측에서 개입으로 넘어간다.
전부 loopback 안에서 벌어지는 자가 검증이라 외부 대상이 없다.

형식은 01~07 과 동일하다 — 빈칸을 채우고 `make NN-check` 로 채점받고,
통과하면 `STUDY.html` 로 이론을 정리한다.
...
└── STUDY.html            ← 이론 + 동작 해부 + OX 퀴즈 (채점 통과 후)
```
...
|---|---|
| `make list` | 예제 목록 |
| `make 01` ~ `make 14` | 예제/게임 실행 |
| `make 07 ARGS="-json"` | 인자 전달 |
| `make noise` | 이벤트를 인위적으로 발생 (다른 터미널에서) |
| `make game` | 12 안티치트용 게임 타깃 빌드 |
| `make workload` | 04 uprobe 타깃 빌드 |

### 채점 (00~14 전부)

| 명령 | 설명 |
|---|---|
| `make hunt` / `make hunt-check` | 00 훅 사냥 — 코드 없이 훅만 찾고 채점 |
| `make 01-check` ~ `make 07-check` | 기본기 트랙 단계별 채점 (`02-check` 등, 06 까지) |
| `make 05-check` | XDP 실습형 채점 |
| `make 09-check` ~ `make 14-check` | 보안 게임 채점 |
| `make study` | 각 랩의 STUDY.html 목록 |

모든 랩이 같은 규칙이다 — `examples/NN-*/` 의 `.bpf.c` 🎯 TODO 를 **Lv1 부터 순서대로**
채우고 `make NN-check` 로 채점한다. **통과 기준 70점.** 앞 단계를 통과해야 뒤 단계를
채점하고, 막히면 같은 폴더의 `solution/` 을 본다.
채점을 통과하면 `examples/NN-*/STUDY.html` 에서 이론 정리 + OX 퀴즈로 마무리한다.
