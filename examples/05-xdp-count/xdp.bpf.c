...
int xdp_count(struct xdp_md *ctx)
{
	...
	/* ==================================================================
	 * 🎯 실습 TODO - IPv4 패킷을 프로토콜별로 세어라.
	 *
	 *   여기까지 오면 이건 IPv4 패킷이다. IP 헤더를 읽어 protocol 필드
	 *   (6=TCP, 17=UDP, 1=ICMP)로 카운트하면 된다. 단, XDP 는 접근 전
	 *   반드시 경계 검사를 해야 verifier 가 통과시킨다.
	 *
	 *   할 일:
	 *     struct iphdr *ip = (void *)(eth + 1);
	 *     if ((void *)(ip + 1) > data_end)   // 이 경계 검사가 없으면 로드 거부!
	 *         return XDP_PASS;
	 *     count(ip->protocol, len);
	 *
	 *   채우기 전엔 IPv4 카운트가 안 잡힌다. 채우면 make 05 에서 TCP/ICMP 가 보인다.
	 *   막히면 solution/xdp.bpf.c 참고.
	 *
	 *   💡 확장(직접): TCP 헤더까지 파싱해 포트별로, 또는 특정 IP 만.
	 * ================================================================== */

	return XDP_PASS; /* 지금은 IPv4 를 세지 않는다 (TODO 를 채워라) */
}
