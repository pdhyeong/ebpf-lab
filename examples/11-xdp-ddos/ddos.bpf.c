...
/*
 * 11. XDP DDoS 방어 게임 (게임판 - TODO 를 채워라) - 드라이버 최전선에서 공격을 떨군다
 *
...
int ddos_filter(struct xdp_md *ctx)
{
	...
	/* ==================================================================
	 * TODO - 출발지별 초당 SYN 속도를 재고, 임계값을 넘으면 XDP_DROP.
	 *
	 *   준비된 것:
	 *     - rates : LRU 해시맵 (키=출발지 IP, 값=struct rate{window_start,syn_count,blocked})
	 *     - cfg->syn_per_sec : 초당 임계값
	 *     - bpf_ktime_get_ns(), NS_PER_SEC 상수
	 *
	 *   할 일:
	 *     1) __u32 saddr = ip->saddr;  __u64 now = bpf_ktime_get_ns();
	 *     2) r = bpf_map_lookup_elem(&rates, &saddr);
	 *        - 없으면: 새 rate{now,1,0} 를 넣고 XDP_PASS
	 *     3) now - r->window_start >= NS_PER_SEC 이면 창 리셋(count=1) 후 XDP_PASS
	 *     4) 아니면 r->syn_count++;
	 *     5) r->syn_count > cfg->syn_per_sec 이면
	 *          bump(1); return XDP_DROP;   // 공격자 차단
	 *     6) 그 외 XDP_PASS
	 *
	 *   막히면 solution/ddos.bpf.c 참고.
	 *
	 *   💡 확장 아이디어(직접): 창을 슬라이딩 윈도로, 차단 IP 를 별도 blocklist
	 *      맵에 넣어 일정 시간 밴, SYN 뿐 아니라 UDP flood 도 탐지 등.
	 * ================================================================== */

	return XDP_PASS; /* 지금은 전부 통과 (아무것도 안 막는다) */
}
