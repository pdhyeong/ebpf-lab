...
/*
 * 13. 미니 백신 (게임판 - TODO 를 채워라) - 시그니처 기반 파일 접근 차단
 *
...
int BPF_PROG(av_scan, struct file *file, int ret)
{
	...
	bump(0); /* 검사한 open 수 (이건 이미 세어 준다) */

	/* ==================================================================
	 * TODO - 파일명을 읽어 시그니처(".virus")가 있으면 격리(-EPERM).
	 *
	 *   1) 파일명 읽기 (CO-RE 로 file->f_path.dentry->d_name.name):
	 *        char name[NAME_LEN] = {};
	 *        struct dentry *dentry = BPF_CORE_READ(file, f_path.dentry);
	 *        const unsigned char *dn = BPF_CORE_READ(dentry, d_name.name);
	 *        bpf_probe_read_kernel_str(name, sizeof(name), dn);
	 *
	 *   2) 시그니처 매칭: has_virus_sig(name) 가 이미 위에 구현돼 있다.
	 *      (원한다면 직접 다른 시그니처로 바꿔도 된다)
	 *
	 *   3) 매치되면:
	 *        bump(1);
	 *        return -1;   // -EPERM => 커널이 파일 열기를 거부
	 *
	 *   막히면 solution/av.bpf.c 참고.
	 *
	 *   💡 확장 아이디어(직접): 확장자 목록, 경로 화이트리스트, 그리고
	 *      진짜 백신처럼 파일 앞부분 매직바이트(예: MZ, ELF)까지 검사.
	 * ================================================================== */

	return 0; /* 지금은 전부 허용 (아무것도 격리 안 함) */
}
